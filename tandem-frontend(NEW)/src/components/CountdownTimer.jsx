...
const CountdownTimer = ({
  ...
  const startMsProp = useMemo(() => (startTime ? new Date(startTime).getTime() : undefined), [startTime])
  ...
  const dayMs = 24 * 60 * 60 * 1000
  const fiveMinMs = 5 * 60 * 1000
  // ceil so 8.01 days -> 9 (starts at 09 as requested)
  let remainingDays = Math.max(0, Math.ceil(remainingMs / dayMs))
  let daysText = String(remainingDays).padStart(2, '0')
  if (isNaN(remainingDays) || daysText === 'NaN' || daysText === '00') {
    daysText = '09'
  }
  ...
  // Meter decreases every 5 minutes; days text stays whole days
  ...
  background,
    ...
    >
      {/* Content */}
      <div className="flex flex-col items-center leading-none select-none" style={{ gap: 2 }}>
        <div
          style={{
            fontSize: 64,
            fontWeight: 700,
      ...
      <svg
        ...
        aria-hidden="true"
      >
        {/* Track (faint) - full rounded rectangle starting at top-right, anticlockwise */}
    <path
          d={`M ${W - R - SW/2},${SW/2}
              L ${R + SW/2},${SW/2}
